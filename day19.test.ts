...
const parseRules = (input: string): Rules =>
  new Map<number, Rule>(
    input.split("\n").map((line) => {
      const [id, rest] = line.split(": ");

      if (rest[0] === '"') return [Number(id), rest[1]];

      return [
        Number(id),
        rest.split(" | ").map((refs) => refs.split(" ").map(Number)),
      ];
    })
  );
...
const matches = (
  ...
  return rule.flatMap((refs) =>
    refs.reduce(
      (acc, ref) => acc.flatMap((i) => matches(rules, message, ref, i)),
      [index]
    )
  );
};
...
test("Day 19a - test", () => {
  const { rules, messages } = testInput;
...
const testInput = prepareInput`0: 4 1 5
1: 2 3 | 3 2
2: 4 4 | 5 5
3: 4 5 | 5 4
4: "a"
...
const prodInput = prepareInput`62: 93 93
41: 40 111 | 127 70
95: 127 93 | 40 40
73: 127 24 | 40 58
121: 127 82 | 40 75
