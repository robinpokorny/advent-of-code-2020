...
/* === PREPARE INPUT === */
const re = /^(acc|jmp|nop) \+?(-?\d+)/;
const parseLine = (line: string): Command => {
  const [, name, arg] = line.match(re) ?? [];
...
const runUntilLoop: RunUntilLoop = (
  commands,
  value,
  lineNo = 0,
  run = new Set()
  ...
  switch (name) {
    case "nop":
      return runUntilLoop(commands, value, lineNo + 1, run);
    case "jmp":
      return runUntilLoop(commands, value, lineNo + arg, run);
    case "acc":
      return runUntilLoop(commands, value + arg, lineNo + 1, run);
  }
};
...
test("Day 8a - test", () => {
  const result = runUntilLoop(testInput, 0);

  expect(result).toBe(5);
});

test("Day 8a - prod", () => {
  const result = runUntilLoop(prodInput, 0);
...
const testInput = prepareInput`nop +0
acc +1
jmp +4
acc +3
jmp -3
...
const prodInput = prepareInput`nop +346
acc +44
acc +15
jmp +473
acc +29
