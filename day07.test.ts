...
const re = new RegExp(
  `^(\\w+ \\w+) bags contain (no other bags)?${bags}${bags}${bags}${bags}\\.`
);
const parseLine = (line: string): Bag => {
  // const [, from, to, letter, password] =
  ...
  const contains = {
    ...
    ...(countB && { [idB]: Number(countB) }),
    ...(countC && { [idC]: Number(countC) }),
...
const canBeInAll = (bags: Bag[], bag: string) => {
  ...
  while (process.length > 0) {
    ...
    process.push(...unprocessedParents);
  }

  // do not count the original bag
  outer.delete(bag);
...
const testInput = prepareInput`light red bags contain 1 bright white bag, 2 muted yellow bags.
